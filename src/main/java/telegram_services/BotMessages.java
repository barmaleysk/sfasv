...
public enum  BotMessages {
    ...
    FAQ("*О «ПРОЕКТЕ»*\n" +
            ...
            "_https://www.cryptopia.co.nz/_\n" +
            "_https://www.bitfinex.com_\n" +
            "*8 Обязательно вводить номер кошелька advcash?*\n" +
            "_Если Вы оплачивали подписку с того кошелька, на который хотите получать реферальное вознаграждение – не обязательно, все произойдет автоматически, в противном случае нужно внести данные о кошельке в разделе настроек бота._\n" +
            "*ПРО ОПЛАТУ*\n" +
            ...
            "_Можно, только в случае если вы сделаете это до того, как у вас появятся свои рефералы_\n" +
            "*17 Как мне заказать выплату бонусов?*\n" +
            "_Вы можете заказать выплату бонусов в подразделе «Мои бонусы» партнерской программы, выплата бонусов происходит по выходным, с возможным отклонением до понедельника – вторника. Специфика вывода такова, что вы можете заказать только вывод всех бонусов, накопившихся с последней выплаты, учтется сумма, актуальная на момент выплаты._\n"+
            "*Общие вопросы*\n" +
            "*18 Выгоднее покупать сигналы на бОльший срок?*\n" +
            "_Да,  пакет 600$мес*12 мес = 7200$ в год, пакет 800$ 2 мес * 12 мес = 4800$ в год, пакет 1000$ 3 мес = 4000$ в год_\n"+
            "*19 Как воспользоваться услугой «Аудит портфеля»?*\n" +
            "_Для того, чтобы получить консультацию по вашему портфелю, рекомендуем Вам сначала заполнить профиль в системе CryptoCompare, это значительно упростит работу аналитика, затем произведите оплату услуги «Аудит портфеля», после оплаты снова выберите услугу «Аудит портфеля», у Вас отобразится активная кнопка «запросить консультацию», после ее нажатия ожидайте приглашения в личный чат с консультантом._"),
    HOW_TO_CHANGE_CURRENCY("Если Вам требуется обмен криптовалюты, вы можете обратиться к @ich333,@Sorokrs, или @Jahmaiker"),
    SUPPORT("Если Вам требуется техподдержка, Вы можете обратиться к @ich333 или @ShakhLeo" ),
    ...
    ONE_MONTH("АКЦИЯ! Стоимость подписки на 1 месяц = $300" +
            "\nНажмите \"Оплатить\", чтобы перейти к оплате  на сайте Advcash"),
    TWO_MONTH("АКЦИЯ! Стоимость подписки на 2 месяца = $400"
            +"\nНажмите \"Оплатить\", чтобы перейти к оплате  на сайте Advcash"),
    THREE_MONTH("АКЦИЯ! Стоимость подписки на 3 месяца  = $500"
            +"\nНажмите \"Оплатить\", чтобы перейти к оплате  на сайте Advcash"),
    UNLIMIT_SUBSCRIPTION("АКЦИЯ! Стоимость безлимитной подписки = $5000"
            +"\nНажмите \"Оплатить\", чтобы перейти к оплате  на сайте Advcash"),
